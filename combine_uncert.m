...
loc='LIN'; 
years=[2005 2006 2008 2009 2010 2011 2012 2013 2014]; 
...
for iy=1:nyears
   year=years(iy); 
   rfile = [loc ,'_AIRS_',num2str(year),'_uncert.rtp'];
   [h_year ha_year p_year pa_year] = rtpread(rfile); 
   [junk nobs]=size(p_year.plat);
...
%   p_all.nemis(nobs_tot-nobs+1:nobs_tot)=p_year.nemis(1:nobs);
   ...
   [num_levs nobs]=size(p_year.ptemp);
   p_all.plevs(:,nobs_tot-nobs+1:nobs_tot)=p_year.plevs(:,1:nobs);
%   p_all.palts(:,nobs_tot-nobs+1:nobs_tot)=p_year.palts(:,1:nobs);
   p_all.ptemp(1:num_levs,nobs_tot-nobs+1:nobs_tot)=p_year.ptemp(1:num_levs,1:nobs);
%   pall.upwell = ones(size(pall.rlon)); 
   p_all.gas_1(1:num_levs,nobs_tot-nobs+1:nobs_tot)=p_year.gas_1(1:num_levs,1:nobs);
...
%   p_all.upwell(nobs_tot-nobs+1:nobs_tot)=ones(size(p_year.zobs)); %p_year.upwell(1:nobs);
...
%   pall.upwell = ones(size(pall.rlon));
   ...
   wfile = [loc,'/',loc ,'_AIRS_uncert.rtp'];
